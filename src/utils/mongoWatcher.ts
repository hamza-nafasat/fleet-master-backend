...
const doneAllFuncOnOneData = async (sensor: any) => {
  const truckId = sensor.truckId;
  const ownerId = sensor.ownerId;
  const truckLatitude = sensor.gps_latitude;
  ...
  let truckFullData: any;
  if (watchPolygonTrucksData.has(String(truckId)) && device?._id) {
    console.log("hsdjf;kalsjkd;flajs;jdkjf;alsmd;ofijaspo;dnmf;laksjodfijasodjfa;skdfoapis");
    const updateTruckPromise = Truck.findOneAndUpdate(
      {
        $and: [{ _id: truckId }, { ownerId: ownerId }],
      },
      { latitude: truckLatitude, longitude: truckLongitude },
      { new: true }
    );
...
export { notificationWatcher, sensorWatcher, doneAllFuncOnOneData };
